...
# https://github.com/vencis/docker-zenoss4
############################################################
...
## init build env & install apt deps
    ...
    && /bin/echo 'deb http://archive.debian.org/debian wheezy main' > /etc/apt/sources.list \
    && /bin/echo 'deb http://archive.debian.org/debian-security wheezy/updates main' >> /etc/apt/sources.list \
    && /usr/bin/apt-get update -o Acquire::Check-Valid-Until=false \
...
##    && /bin/sed 's/wheezy/jessie/g' /etc/apt/sources.list >> /etc/apt/sources.list \
    && /bin/echo 'deb http://deb.debian.org/debian/ jessie main' > /etc/apt/sources.list \
    && /bin/echo 'deb http://security.debian.org/ jessie/updates main' >> /etc/apt/sources.list \
    && /usr/bin/apt-get update -o Acquire::Check-Valid-Until=false \
...
## download all the things!
    && /usr/bin/wget --no-check-certificate https://raw.githubusercontent.com/vencis/docker-misc/master/init_fs/usr/local/src/snmp-mibs-downloader_1.1_all.deb -O ${BUILD_SRC}/snmp-mibs-downloader_1.1_all.deb \
    && /usr/bin/wget --no-check-certificate https://raw.githubusercontent.com/vencis/docker-misc/master/init_fs/usr/local/src/zenoss-core-425-2108_03c_amd64.deb.tar.gz.00 -O ${BUILD_SRC}/zenoss-core-425-2108_03c_amd64.deb.tar.gz.00 \
    && /usr/bin/wget --no-check-certificate https://raw.githubusercontent.com/vencis/docker-misc/master/init_fs/usr/local/src/zenoss-core-425-2108_03c_amd64.deb.tar.gz.01 -O ${BUILD_SRC}/zenoss-core-425-2108_03c_amd64.deb.tar.gz.01 \
    ...
    && /usr/bin/wget --no-check-certificate https://raw.githubusercontent.com/vencis/docker-misc/master/init_fs/usr/local/bin/docker-entrypoint.sh -O /usr/local/bin/docker-entrypoint.sh \
    && /usr/bin/wget --no-check-certificate https://raw.githubusercontent.com/vencis/docker-misc/master/init_fs/etc/insserv/overrides/zenoss -O /etc/insserv/overrides/zenoss \
    && /usr/bin/wget --no-check-certificate https://raw.githubusercontent.com/vencis/docker-misc/master/init_fs/etc/init.d/zenoss -O /etc/init.d/zenoss \
...
## install all the things!
    ...
    && /usr/bin/wget --no-check-certificate https://raw.githubusercontent.com/vencis/docker-misc/master/init_fs/usr/local/zenoss/etc/zeneventserver.conf -O ${ZENHOME}/etc/zeneventserver.conf \
